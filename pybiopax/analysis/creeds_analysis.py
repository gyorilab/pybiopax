...
from indra.statements import (
    Agent, RegulateAmount, Statement, stmts_to_json_file,
)
...
def get_reactome_human_ids() -> set[str]:
    ...
    rv = {
        reactome_id
        for reactome_id in identifiers
        if species[reactome_id] == "9606"
    }
    return set(sorted(rv)[:5])
...
def get_protein_hgnc(protein: Protein) -> Optional[str]:
    if protein.entity_reference is None:
        return None
    rv = {
        bioregistry.normalize_prefix(xref.db): xref.id
        for xref in protein.entity_reference.xref
    }
    hgnc_id = rv.get("hgnc")
...
def ensure_reactome(reactome_id: str, force: bool = False):
    ...
    if path.is_file() and not force:
        return pybiopax.model_from_owl_file(path)
...
def _main():
    ...
    for pert_id, pert_genes in tqdm(creeds_drug_hgncs.items()):
        ...
        for reactome_id, reactome_genes in tqdm(reactome_it, leave=False):
            ...
            rows.append((f'hgnc:{pert_id}', reactome_id, p_value))
